...
class Morphism(nn.Module):
    def _init_(self, name = 'Morphisme R^n --> E', dim_E = 1, neurons = 6):
        print(f'[Model] name : {name}')
        print(f'[Model] dim E : {dim_E}')
        print(f'[Model] no. neurons per layers : {neurons}')
        super(Morphism, self)._init_()
        # layers for plus : E --> E
...
class InverseMorphism(nn.Module):
    def _init_(self, name = 'Inverse E --> R^n', dim_E = 1, neurons = 6):
        print(f'[Model] name : {name}')
        print(f'[Model] dim E : {dim_E}')
        print(f'[Model] no. neurons per layers : {neurons}')
        super(InverseMorphism, self)._init_()
        # layers for plus : E --> E
        self.fc1 = nn.Linear(dim_E, neurons)
        self.fc2 = nn.Linear(neurons, neurons)
        self.fc3 = nn.Linear(neurons, dim_E)
...
class LoiBinaire(nn.Module):
    def _init_(self, name = 'Loi binaire ExE-->E', dim_E = 1, neurons = 6):
        print(f'[Model] name : {name}')
        print(f'[Model] dim E : {dim_E}')
        print(f'[Model] no. neurons per layers : {neurons}')
        super(LoiBinaire, self)._init_()
...
class LoiScalaire(nn.Module):
    def _init_(self, name = 'Loi Scalaire RxE-->E', dim_E = 1, neurons = 6):
        print(f'[Model] name : {name}')
        print(f'[Model] dim E : {dim_E}')
        print(f'[Model] no. neurons per layers : {neurons}')
        super(LoiScalaire, self)._init_()
        # layers for plus : ExE --> E
        self.fc1 = nn.Linear(dim_E, neurons)
        self.fc2 = nn.Linear(neurons, neurons)
        self.fc3 = nn.Linear(neurons, dim_E)
    def forward(self, alpha, x):
        z = torch.cat([x], axis=1) # [K,d], [K,d] ---> [K, 2*d]
        z = self.fc1(z)
        z = self.fc2(z)
        output = self.fc3(z)
        return output
class Group(nn.Module):
    def _init_(self, K,  dim_E = 2 , neurons = 6, name = 'Groupe (E,+)'):
        super(Group, self)._init_()
        ...
        self.loss_3 = lambda x, y : torch.linalg.vector_norm(x - y)  + torch.linalg.vector_norm(self.fi( self.f(x))- self.fi( self.f(y) ))
        # # losses
        ...
        self.loss  = lambda x,y:  self.loss_1(x,y)
        self.loss  = lambda x,alpha:  self.loss_2(alpha,x)
        self.loss  = lambda x,y:  self.loss_3(x,y)
        self.loss  = lambda x,y,alpha:  self.loss_1(x,y) + self.loss_2(alpha,x) + self.loss_3(x,y)
    def train(self, X, Y,alpha, optimizer, epoch):
        ...
        for i in range(epoch):
            ...
            loss = L1 + L2 + L3
            #if i % 10 == 0:
            print('Epoch {}/{} -\t Loss 1: {:.6f}\t Loss 2: {:.6f}\t Loss 3: {:.6f}\t Total Loss: {:.6f}'
                      .format(i, epoch, L1.item(), L2.item(),L3.item(), loss.item()))
            loss.backward(retain_graph=True)
            optimizer.step()
    def test(self, test_loader):
        pass
# Dataset generation
def elliptic_curve(K, a=2, b=3):
    X = 0.3*torch.randn(K, 2).requires_grad_(False)
    Y = 0.3*torch.randn(K, 2).requires_grad_(False)
    alpha = 0.3*torch.randn(K, 1).requires_grad_(False)
    X[:,1] = X[:,0] + torch.sin(X[:,0]) # **3 + a*X[:,0] + b
    Y[:,1] = Y[:,0] + torch.sin(Y[:,0]) #**3 + a*Y[:,0] + b
    alpha = alpha #**3 + a*alpha + b
    return X, Y, alpha
