...
\section{Objectives}
    ...
    \subsection{Sub Objectives}
        ...
        \subsection*{Third sub objectif : 2D case}
...
\section{Tools}
    ...
    \begin{enumerate}
        ...
        \[
            L_1(\theta) = \sum_{(x, y) \in V \times V} \left\lVert_{V} x \oplus y - \left(f(f^{-1}(x)) + f(f^{-1}(y))\right) \right\rVert_{V}^2
        \]
        \item Loss for scalar multiplication:
        \[
            L_2(\theta) = \sum_{(\alpha, x) \in \mathbb{R} \times V} \left\lVert_{V} \alpha \odot x - f(\alpha \cdot f^{-1}(x)) \right\rVert_{V}^2
        \]
    \end{enumerate}
    ...
    We  precise that the norm $\left\lVert_{V} \cdot \right\rVert_{V}$ is the norm defined on the dataset $V$.


    We will use Neural Networks Learning to find the best couple that minimize the loss function $L(\theta)$.
    ...
    \subsection{Numerical Experiments}
        \subsubsection{One Dimensional Case}
            ...
            \begin{itemize}
                ...
                \item $\varepsilon = 0.1$
                \item $dim_E = 2$ like the dataset is defined on a 1D 
                        surface have 2 dimensionsn, the abscissa and the ordinate.
                \item The Number of Epochs = 64  to train the model.
                \item The learning rate = 0.1
            \end{itemize}
        % -----------------------------
        \subsubsection{Results}
            ...
            \begin{table}[h]
                ...
                \hline
                0/64 & 77.023567 & 80.262703 & 157.286270 \\
                10/64 & 4.763151 & 10.016760 & 14.779911 \\
                20/64 & 0.176072 & 1.071531 & 1.247603 \\
                30/64 & 0.029569 & 0.199473 & 0.229042 \\
                40/64 & 0.015680 & 0.068417 & 0.084097 \\
                50/64 & 0.010941 & 0.019243 & 0.030184 \\
                60/64 & 0.009359 & 0.003955 & 0.013314 \\
                \hline
                \end{tabular}
            ...
            \begin{figure}[h]
                ...
                \begin{subfigure}{0.4\textwidth}
                    \centering
                    \includegraphics[width=\linewidth]{./images/M.png}
                    \caption{X\_train}
                    \label{fig:X_train}
                \end{subfigure}
                \hfill
                \begin{subfigure}{0.4\textwidth}
                    ...
                    \caption{X\_test, epsilon = 0.1}
                    \label{fig:X_test}
                \end{subfigure}
                \caption{Representations of X\_train and X\_test}
                \label{fig:data_representations}
            \end{figure}
            ...
            \textbf{The model is able to accurately predict the dataset, as shown by the close alignment between the predicted and actual values. This indicates that the model has learned the underlying structure of the dataset and can generate accurate predictions based on the input data.}

            

            We chose the dataset $(\mathbf{x}, \mathbf{x}+\varepsilon \sin (\mathbf{x} / \varepsilon))$ with $\varepsilon = 0.1$ and $\mathbf{x}$ randomly selected following a normal distribution with mean 0 and variance 1.
            \\

            Another representation that we we can have :
            \begin{figure}[h]
                \centering
                \begin{subfigure}{0.4\textwidth}
                    \centering
                    \includegraphics[width=\linewidth]{./images/test.png}
                    \caption{X\_test, epsilon = 0.1, this one is almost a line with small variations}
                \end{subfigure}
                \hfill
                \begin{subfigure}{0.4\textwidth}
                    \centering
                    ...
                    \caption{X\_test, epsilon = 0.1}
                \end{subfigure}
                \caption{Representations of various solutions}
            \end{figure}

            The explanation we can bring to table is this one :
            we precise that the Solution is not unique, there are many solutions that can satisfy the conditions defined in the first sub-objective.
